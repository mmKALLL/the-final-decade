  ...
  return (
    <div className="game-over-screen">
      ...
      <p>
        {gs.money <= 0
          ? gs.language === 'en-US'
            ? `Your organization failed to remain profitable and went bankrupt on ${getDateFromTurn(gs.turn)}.`
            : `${getDateFromTurn(gs.turn)}までに利益を維持できず、倒産しました。`
          : gs.asiOutcome <= 0
          ? gs.language === 'en-US'
            ? `Your organization was shut down on ${getDateFromTurn(
                gs.turn
              )}, as the public has taken a careless attitude towards AI alignment.`
            : `あなたの会社は${getDateFromTurn(gs.turn)}に、公衆がAIの整合性に対する無頓着な態度の結果として閉鎖されました。`
          : gs.trust <= 0
          ? gs.language === 'en-US'
            ? `Your organization failed to maintain trust with your stakeholders on ${getDateFromTurn(gs.turn)}.`
            : `${getDateFromTurn(gs.turn)}までに従業員との信頼関係を維持できませんでした。`
          : gs.language === 'en-US'
          ? `Your organization failed to meet the annual breakthrough goals by ${getDateFromTurn(gs.turn)}.`
